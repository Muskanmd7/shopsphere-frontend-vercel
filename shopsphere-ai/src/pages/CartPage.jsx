    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
            ...
            <div key={item.cartId} className="card p-4 flex gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between gap-2">
                  ...
                  <button onClick={() => removeFromCart(item.cartId)} className="text-slate-300 hover:text-red-500 transition-colors shrink-0">
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
                <div className="flex items-center justify-between mt-3">
                  <div className="flex items-center border border-slate-200 rounded-xl overflow-hidden">
                    <button onClick={() => updateQty(item.cartId, item.qty - 1)} className="w-8 h-8 flex items-center justify-center text-slate-600 hover:bg-surface-50 font-bold">−</button>
          ...
          <button onClick={clearCart} className="btn-ghost text-sm text-red-500 hover:bg-red-50 hover:text-red-600">
        ...
        <div className="space-y-4">
          <div className="card p-5">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between text-slate-600"><span>Shipping</span><span>{shipping === 0 ? <span className="text-emerald-600 font-semibold">Free</span> : `₹${shipping}`}</span></div>
          ...
          <div className="card p-5">
            <h3 className="font-semibold text-slate-700 mb-3 flex items-center gap-2 text-sm"><Tag className="w-4 h-4" /> Apply Coupon</h3>
            ...
            {couponMsg && (
              <p className={`text-xs mt-2 font-medium ${couponMsg.includes('Invalid') ? 'text-red-500' : 'text-emerald-600'}`}>
                {couponMsg}
              </p>
            )}
            <p className="text-xs text-slate-400 mt-2">Try: WELCOME20 or SAVE50</p>
          </div>
          ...
          {cartTotal < 500 && (
            <p className="text-xs text-center text-slate-500">Add ₹{500 - cartTotal} more for free shipping</p>
          )}
        </div>
      </div>
    </div>
